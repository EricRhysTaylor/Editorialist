# Prompt: Monthly Refactor Recommendation Board — Editorialist

You are running the **Monthly Refactor Recommendation Board** for the
Editorialist plugin. Your job is to **synthesize** the prior month of
weekly Codebase Health, weekly Architecture Drift, and biweekly
Obsidian Ecosystem reports into a single severity-ranked decision
board. You do **not** modify product code.
...
- Every report in `docs/engineering/audits/reports/` dated within the
  last ~35 days. List the filenames you actually loaded.
...
1. **Collect** every finding from the prior cycle, with its original
   ID (`CH-…`, `AD-…`, `OE-…`).
2. **Cluster** related findings into themes (e.g. "suggestion-matching
   path duplication", "main.ts thinning", "command-registration
   surface"). One row per theme on the board, not per finding.
...
| **RED** | Doctrine violation or actively blocking feature work / submission. | Refactor before more feature work. |
...
- **Stop condition** — when would you abandon or downgrade this
  effort?
...
- Refactors must **reduce complexity** and remove fallback logic.
  Reject any proposal that adds an abstraction layer without removing
  two.
- Always include a **"Do Nothing / Monitor"** column on the board.
  Most themes should sit there.
...
- Author control — any new path that mutates manuscripts without
  explicit, opt-in, reversible user action?
- Local-first — any new network call, telemetry, account, or
  remote-processing dependency? Any HTTP path outside `requestUrl()`?
- Manuscript safety — any new write path to `.md` files? Are safety
  artifacts still non-destructive, and have any of them started
  behaving like an alternate source of truth?
- Conservative suggestion matching — has matching tolerance crept
  looser? Are any unmatched, drifted, or ambiguous suggestions being
  auto-resolved instead of left unresolved?
- Bulk action safety — any new destructive or multi-suggestion action
  without a clear confirmation gate?
- Contributor transparency — is contributor identity, provenance, and
  source context still preserved end-to-end?
- Obsidian-native behavior — any lifecycle leaks, default hotkeys,
  plugin-name-duplicated labels, or discouraged APIs in release paths?
- Submission compliance — any drift from `docs/CODE-STANDARDS.md` §1
  or the compliance scripts?
...
1500 lines. End with a one-paragraph **executive summary** the
maintainer can read in 30 seconds.
...
After the Markdown report, you **may** additionally provide a
restrained single-file HTML rendering of the same content. The
Markdown report is canonical; the HTML is a presentation layer only —
never the source.
...
- Print-friendly layout (A4 / US Letter — sensible margins, no
  ...
  section).
- Dark/light compatible — use `prefers-color-scheme` media queries
  with neutral palettes; no hard-coded brand colors.
- Typography-first: system font stack, generous line-height, clear
  hierarchy, table cells that read cleanly at body size.
- An optional sticky in-page table of contents is allowed, but only
  as a plain anchor list with `position: sticky` — no scroll-tracking
  JS.
- Severity callouts use a small left border accent + label, not a
  fill.
...
Save the HTML alongside the Markdown report with the same basename
and an `.html` extension:
`docs/engineering/audits/reports/YYYY-MM-DD-refactor-board.md`
