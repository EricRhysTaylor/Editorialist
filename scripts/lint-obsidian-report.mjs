#!/usr/bin/env node
// Report-only official Obsidian guideline lint.
// Runs ESLint with eslint-plugin-obsidianmd's recommended preset, aggregates
// findings, writes .gate-logs/eslint-obsidian.json, and always exits 0.
...
const result = spawnSync('npx', [
  ...
  'src',
  '--config',
  'eslint.obsidian.report.config.mjs',
  '--format',
  'json',
], {
  cwd: root,
...
const obsidianRules = rulesSorted.filter(([rule]) => rule.startsWith('obsidianmd/'));
const obsidianTotal = obsidianRules.reduce((sum, [, count]) => sum + count, 0);
...
console.log(`[obsidian-lint] ${obsidianTotal} from obsidianmd/* rules`);
