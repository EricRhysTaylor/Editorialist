#!/usr/bin/env node
// Blocking, ratcheted official Obsidian lint subset.
//
// Existing findings are budgeted in
// scripts/eslint-obsidian-enforced-baseline.json. The gate fails only when a
// selected rule's count increases.
...
try {
  ...
  if (updateBaseline) {
    const payload = baselinePayload(summary);
